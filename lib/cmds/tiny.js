...
exports.handler = function (argv) { return __awaiter(_this, void 0, void 0, function () {
    ...
    return __generator(this, function (_b) {
        ...
        files.forEach(function (file) { return __awaiter(_this, void 0, void 0, function () {
            var _a, path, name, hash, blueName, tinyPath_1, tinyData_1, _b, input_1, output_1, writeS, error_1;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    ...
                    case 1:
                        _c.trys.push([1, 6, , 7]);
                        tinyPath_1 = path + "/.tinyrc";
                        if (!shelljs_1.test('-f', tinyPath_1))
                            fs_1.appendFileSync(tinyPath_1, '{}', { flag: 'w' });
                        tinyData_1 = JSON.parse(shelljs_1.cat(tinyPath_1));
                        if (!utils_1.hasUpload(name, tinyData_1, hash)) return [3 /*break*/, 2];
                        spinner.succeed("\u538B\u7F29\u6210\u529F " + blueName);
                        return [3 /*break*/, 5];
                    case 2: return [4 /*yield*/, tinyImg({ path: path, name: name })];
                    case 3:
                        _b = _c.sent(), input_1 = _b.input, output_1 = _b.output;
                        writeS = fs_1.createWriteStream(path + "/" + name);
                        writeS.on('close', function () {
                            var compressHash = utils_1.getEtag(path + "/" + name);
                            tinyData_1 = JSON.parse(shelljs_1.cat(tinyPath_1));
                            tinyData_1["" + name] = { hash: compressHash };
                            fs_1.appendFileSync(tinyPath_1, JSON.stringify(tinyData_1, null, '\t'), { flag: 'w' });
                            spinner.succeed("\u538B\u7F29\u6210\u529F " + blueName).start().info("\u538B\u7F29\u5927\u5C0F: " + (input_1.size / 1024).toFixed(2) + "KB  => " + (output_1.size / 1024).toFixed(2) + "KB");
                        });
                        return [4 /*yield*/, downImg(output_1.url, writeS)];
                    case 4:
                        _c.sent();
                        _c.label = 5;
